...
项目主要使用 Python 和 PyTorch 实现。

主要技术包括：

- **Python**：实验、训练脚本和数据处理工具。
- **PyTorch**：模型定义、训练循环、loss 和 tensor 计算。
- **torchaudio** 和 **soundfile**：音频读取、重采样和基础音频处理。
- **Hydra** 和 **OmegaConf**：autoencoder 实验配置管理。
- **NumPy**：数据处理、统计分析和诊断工具。
- **Matplotlib / Librosa**：频谱分析、诊断图和可视化。
- **CosyVoice3 相关工具链**：构造合成多说话人中文语音数据。

这是一个研究型仓库，而不是打包好的通用库。部分分析脚本可能会根据具体实验
需要额外依赖。
